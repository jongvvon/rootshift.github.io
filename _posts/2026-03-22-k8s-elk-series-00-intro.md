...
나는 대형 운영 시설의 IT 보안 인프라팀에서 일한다.

담당 시스템 중에 **보안설비관리시스템(이하 SFMS)**이라는 게 있다. 이름은 거창한데 구조를 뜯어보면 딱 ELK 스택이다.

```
현장 장비/서버 (SNMP, 시스템 로그)
    ↓
Logstash AP서버 6대 (수집/파싱)
    ↓
Elasticsearch DB서버 2대 (저장/검색)
    ↓
대시보드 서버 2대 (시각화)
```

총 10대 서버, 물리 장비. 인덱스 1,459개, 데이터 약 1.1TB.
